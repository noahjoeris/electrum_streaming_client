...
use electrum_streaming_client::{
    notification::Notification, request, AsyncClient, CompletedRequest, ConnectConfig, Event,
};
use futures::{FutureExt, StreamExt};

#[tokio::test]
async fn synopsis() -> anyhow::Result<()> {
    let env = TestEnv::new()?;
    let url = format!("tcp://{}", env.electrsd.electrum_url);
